...
private:
    ...
    bool m_was_hit_last_turn = false;
    bool m_prefer_left_side = true;
    bool m_side_selected = false;
    int m_last_health = 100;
...
public:
    Lightheart_Robot() : RobotBase(1, 6, railgun) {} // Initialize with 1 movement, 6 armor, railgun
    ...
    virtual void process_radar_results(const std::vector<RadarObj>& radar_results) override 
    {
        ...
        m_was_hit_last_turn = current_health < m_last_health;
...
void get_move_direction(int& move_direction, int& move_distance) override 
{
    ...
    if (!m_side_selected)
    {
        m_prefer_left_side = current_col <= (m_board_col_max - 1) / 2;
        m_side_selected = true;
    }

    // Step 1: Move horizontally until the chosen side wall is reached.
    if (m_prefer_left_side && current_col > 0) 
    {
        move_direction = 7; // Left
        move_distance = std::min(move, current_col); // Clamp to avoid going out of bounds
        return;
    }
    if (!m_prefer_left_side && current_col < m_board_col_max - 1)
    {
    ...
    // Step 2: Vertical movement once the chosen side is reached.
    if (m_moving_down) 
    {
